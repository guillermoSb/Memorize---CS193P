...
struct GridLayout {
    ...
    init(itemCount: Int, nearAspectRatio desiredAspectRatio: Double = 1, in size: CGSize) {
        ...
        var bestLayout: (rowCount: Int, columnCount: Int) = (1, itemCount)  // Define the possible best layout
        var smallestVariance: Double?
        let sizeAspectRatio = abs(Double(size.width/size.height)) // The size aspect ratio of the container
        for rows in 1...itemCount {
            let columns = (itemCount / rows) + (itemCount % rows > 0 ? 1 : 0) // Define the possible number of columns for each layout.
